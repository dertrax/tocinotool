...
- Cuando un WEB-DL no contiene ningún subtítulo español marcado como forzado,
  un único SRT español completo puede proponer carteles escritos mayoritariamente
  en MAYÚSCULAS. Si existe completo y SDH, se prueba antes el completo; varias
  fuentes del mismo tipo no se adivinan.
- La propuesta se muestra y requiere confirmación explícita. Al aceptarla se
  añade únicamente un SRT `[Forzados]`; el SRT completo/SDH original nunca se
  sustituye. No se usa esta heurística desde WebVTT ni si ya existe un forzado
  español, evitando duplicados o diálogos enfáticos mal etiquetados.

## 2026-09-13 — Publicación 2.1.7

- La ficha deja de producir un NFO de texto duplicado y genera NFO XML de
  identificación. Películas: `<movie>` junto al release; capítulos:
  `<episodedetails>` junto al MKV y `tvshow.nfo`; temporadas completas:
  `tvshow.nfo`, `season.nfo` y un sidecar para cada capítulo.
- Los IDs de TMDb, TheTVDB e IMDb se obtienen de FileBot. Para no falsear los
  metadatos, los IDs de una serie se escriben solo en `tvshow.nfo`: FileBot no
  proporciona IDs individuales de episodio. Sin FileBot, el enlace manual
  mantiene TMDb para películas e IMDb para series.
- `config/tracker.yaml` migra automáticamente la antigua plantilla `nfo:` de
  texto al ajuste `nfo.identificacion: true`; la ficha BBCode continúa siendo
  el documento legible del tracker.
- Subtítulos: los formatos de texto pasan siempre a SRT, tanto si venían
  dentro como fuera del vídeo. En WEB-DL se conserva únicamente el ASS/SSA
  original además de su SRT; Encode/Rip conservan además la fuente original.
  PGS/VobSub exigen OCR y un SRT real: el mux se detiene si falta, sin crear
  texto falso.
- Un WEB-DL con un único ASS/SSA español completo se analiza para carteles
  reales de estilos/posiciones y solo entonces añade SRT+ASS forzados. No se
  inventan forzados desde SRT o WebVTT sin información de typesetting.
